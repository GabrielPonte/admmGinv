...
for inst in instances_array
    ...
    if run_ls
        ...
        @info (string("m = ",m,". LS det: ", round_exact(admmres_lsdet.time,2), " sec. 2,1 norm ", round_exact(admmres_lsdet.norm_21,2), ", 0 norm ", admmres_lsdet.norm_0, ". Iter: ", admmres_lsdet.iter));
        flush(stdout); GC.gc();
        ...
        @info (string("m = ",m,". LS 21: ", round_exact(admmres_ls21.time,2), " sec. 2,1 norm ", round_exact(admmres_ls21.norm_21,2), ", 0 norm ", admmres_ls21.norm_0, ". Iter: ", admmres_ls21.iter));
        flush(stdout); GC.gc();
    end
    # ADMM Initialization
    ...
    if run_admm1n
        if fixed_tol
            ...
            time_admm_1 = @elapsed admmsol_1 = admm1norm(ginvInit;stop_limit=:OptGap);
            admmres_1 = getResultsADMM(inst,admmsol_1);
            writeCSV!(arr_resADMM_1,admmres_1,Symbol(:ADMM_1_,TP))
        else
            # Run ADMM based on the 1-norm
            time_admm_1 = @elapsed admmsol_1 = admm1norm(ginvInit;stop_limit=:Boyd);
            admmres_1 = getResultsADMM(inst,admmsol_1);
            writeCSV!(arr_resADMM_1,admmres_1,Symbol(:ADMM_1_,TP))
        end
        
        @info (string("m = ",m,". ADMM 1: ", round_exact(admmsol_1.time,2), " sec. 1 norm ", round_exact(admmsol_1.z,2), ", 0 norm ", admmres_1.norm_0, ". Iter: ", admmsol_1.iter));
        flush(stdout); GC.gc();
    end
    ...
    if run_admm21n || run_2120n || run_20n
        if fixed_tol
            # Run ADMM based on the 2,1-norm
            time_admm_21 = @elapsed admmsol_21,M_21,Λ_21 = admm21norm(ginvInit;stop_limit=:OptGap);
            admmres_21 = getResultsADMM(inst,admmsol_21);
            if run_admm21n
                writeCSV!(arr_resADMM_21,admmres_21,Symbol(:ADMM_21_,TP))
                @info (string("m = ",m,". ADMM 2,1: ", round_exact(admmsol_21.time,2), " sec. |2,1|: ", round_exact(admmsol_21.z,2), ". |2,0|: ", admmres_21.norm_20, ". |0|: ", admmres_21.norm_0,". Iter: ", admmsol_21.iter));
                flush(stdout); GC.gc();
            end
        else
            # Run ADMM based on the 2,1-norm
            time_admm_21 = @elapsed admmsol_21,M_21,Λ_21 = admm21norm(ginvInit;stop_limit=:Boyd);
            admmres_21 = getResultsADMM(inst,admmsol_21);
            if run_admm21n
                writeCSV!(arr_resADMM_21,admmres_21,Symbol(:ADMM_21_,TP))
                @info (string("m = ",m,". ADMM 2,1: ", round_exact(admmsol_21.time,2), " sec. |2,1|: ", round_exact(admmsol_21.z,2), ". |2,0|: ", admmres_21.norm_20, ". |0|: ", admmres_21.norm_0,". Iter: ", admmsol_21.iter));
        ...
        if (run_2120n || run_20n) && fixed_tol
            ...
            if m >= 1000
                for ω21 in [0.25, 0.50, 0.75, 0.80, 0.90, 0.95]
                    ...
                    if run_20n
                        ...
                        writeCSV!(arr_resADMM_20,admmres_20,Symbol(:ADMM_20))
                        @info (string("w = ",round_exact(ω21,2),". ADMM 2,0: ", round_exact(admmsol_20.time,2), " sec. |2,1|: ", round_exact(admmsol_20.z,2), ". |2,0|: ", admmres_20.norm_20,". |0|: ", admmres_20.norm_0,". Iter: ", admmsol_20.iter));
                        flush(stdout); GC.gc();
                    end
                    if run_2120n
                        time_admm_2120 = @elapsed admmsol_2120 = admm2120norm(ginvInit,ω21,nzr21,M_21,Λ_21);
                        admmres_2120 = getResultsADMM(inst,admmsol_20);
                        writeCSV!(arr_resADMM_2120,admmres_2120,Symbol(:ADMM_21_20))
                        @info (string("w = ",round_exact(ω21,2),".ADMM 2120: ", round_exact(admmsol_2120.time,2), " sec. |2,1|: ", round_exact(admmsol_2120.z,2), ". |2,0|: ", admmres_2120.norm_20,". |0|: ", admmres_2120.norm_0,". Iter: ", admmsol_2120.iter));
                        flush(stdout); GC.gc();
                    end
                end
            end
    ...
    if run_grb 
        ...
        @info (string("m = ",m,". GRB 1: ",  round_exact(grbres_1.time,2), " sec. 1 norm ", round_exact(grbres_1.norm_1,2), ", 0 norm ", grbres_1.norm_0));
        flush(stdout); GC.gc();
    end
    if run_msk
        ...
        @info (string("m = ",m,". MSK 2,1: ", round_exact(mskres_21.time,2), " sec. |2,1|: ", round_exact(mskres_21.norm_21,2), ". |2,0|: ", mskres_21.norm_20, ". |0|: ", mskres_21.norm_0));
        flush(stdout); GC.gc();
    end
end