...
#     GG = V1*U1';
#     # GG = Matrix(I,n,n-r)*U2'
#     Θ0 = (1/maximum(abs.(GG)))*GG
#     Θ1 = copy(Θ0); 

#     E0 = V1DinvU1T + rho_inv*Θ0; 
#     # E0 = rho_inv*Θ0;
#     E1 = copy(E0); 
...
#         # pres[iter] # dres[iter]
#         pres_norm = sqrt(n*m)*eps_abs + eps_rel*max(norm(E),norm(W),norm_V1DinvU1T);
#         dres_norm = sqrt((n-r)*r)*eps_abs + eps_rel*norm(V2'*Θ*U1);
#         push!(tols,min(pres_norm, dres_norm));
        

#         if max(pres[iter],dres[iter]) <= tols[iter] || iter == max_iter || ((time_ns() - time_start)/1e9) >= 7200
