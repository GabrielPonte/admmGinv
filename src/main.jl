    ...
    arr_resADMM_20s = init_ginv_res_admm();
    ...
    for m1 in [1000,2000,3000,4000,5000]
        m = m1;
        ...
        println(string("m = ",m,". LS det finished in ", round_exact(admmres_lsdet.time,2), " sec. 2,1 norm ", round_exact(admmres_lsdet.norm_21,3), ", 0 norm ", admmres_lsdet.norm_0, ". Iter: ", admmres_lsdet.iter));
        # @show lsdetsol.time
        ls21sol= LS_21(A,C,:FI);
        ls21sol.time += lsdetsol.time 
        admmres_ls21 = getResultsADMM(inst,ls21sol);
        println(string("m = ",m,". LS det finished in ", round_exact(admmres_ls21.time,2), " sec. 2,1 norm ", round_exact(admmres_ls21.norm_21,3), ", 0 norm ", admmres_ls21.norm_0, ". Iter: ", admmres_ls21.iter));
        ...
        # # """
        # ω21 = 0.9; nzr21 = admmres_21.NZR;
        # time_admm_20 = @elapsed admmsol_20 = admm20norm(ginvInit,ω21,nzr21,M_21,Λ_21);#runADMM20n(V1Dinv,V2,U1,admmres_21.NZR,ω21,false);
        ...
        # flush(stdout)
        # # @show size(M_21),size(Λ_21)
        # # time_admm_20s = @elapsed admmsol_20s = admm20norm_updt2(ginvInit,ω21,nzr21,M_21,Λ_21;eps_abs=1e-4,eps_rel=1e-4,rho=1e3);#runADMM20n(V1Dinv,V2,U1,admmres_21.NZR,ω21,false);
        # # admmsol_20s.z = getnorm21(admmsol_20s.H);
        # # admmsol_20s.time = time_admm_20s;
        # # admmres_20s = getResultsADMM(inst,admmsol_20s);
        # # writeCSV!(arr_resADMM_20s,admmres_20s,Symbol(:ADMM_2120s_,trunc(Int64,100*ω21)))
        # # println(string("m = ",m,". ADMM 2,0: ", round_exact(admmsol_20s.time,2), " sec. |2,1|: ", round_exact(admmsol_20s.z,3), ". |2,0|: ", admmres_20s.NZR,". |0|: ", admmres_20s.norm_0,". Iter: ", admmsol_20s.iter));
        # # flush(stdout)
