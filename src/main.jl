...
for m1 in [150]#M2
    ...
    # lsdetsol,C = LS_det_not_eff(A,R,deepcopy(C_init),:FI)
    # lsdetsol.time += time_RC 
    # admmres_lsdet = getResultsADMM(inst,lsdetsol);
    # println(string("m = ",m,". LS det finished in ", round_exact(admmres_lsdet.time,2), " sec. 2,1 norm ", round_exact(admmres_lsdet.norm_21,3), ", 0 norm ", admmres_lsdet.norm_0, ". Iter: ", admmres_lsdet.iter));
    # writeCSV!(arr_resLSdet,admmres_lsdet,Symbol(:LS_det_,dens_int))
    # Run local-search based on the 2,1-norm
    ls21sol= LS_21(A,deepcopy(C_init),:FP);
    ls21sol.time += lsdetsol.time 
    admmres_ls21 = getResultsADMM(inst,ls21sol);
    println(string("m = ",m,". LS 21 finished in ", round_exact(admmres_ls21.time,2), " sec. 2,1 norm ", round_exact(admmres_ls21.norm_21,3), ", 0 norm ", admmres_ls21.norm_0, ". Iter: ", admmres_ls21.iter));
    writeCSV!(arr_resLS21,admmres_ls21,Symbol(:LS_21_,dens_int))

    ls21sol= LS_21_not_eff(A,deepcopy(C_init),:FP);
    ls21sol.time += lsdetsol.time 
    admmres_ls21 = getResultsADMM(inst,ls21sol);
    println(string("m = ",m,". LS 21 finished in ", round_exact(admmres_ls21.time,2), " sec. 2,1 norm ", round_exact(admmres_ls21.norm_21,3), ", 0 norm ", admmres_ls21.norm_0, ". Iter: ", admmres_ls21.iter));
    writeCSV!(arr_resLS21,admmres_ls21,Symbol(:LS_21_,dens_int))
