    ...
    while flag>0
        ...
        for column = (1:n-r)
            ...
            for j = (1:r)
                ...
                sum_value = norm_Hj/abs(vj) +  sum(sqrt(sum(Hs.^2, 2) +  2*v_bar.*w +  (v_bar).^2*norm_Hj^2 ));
                check_sum = 0.0;
                for i = (1:r)
                    if i ~= j
                        check_sum = check_sum + sqrt(norm(H_hat(i,:))^2 - 2*(v(i)/v(j))*H_hat(i,:)*H_hat(j,:)' +(v(i)/v(j))^2*norm(H_hat(j,:))^2 );
                    else
                        check_sum = check_sum + norm(H_hat(j,:))/abs(v(j));
                    end
                end
                D = C;
                D(j) = Cb(column);
                
                H_hat_copy(Sj,:) = Hs + v_bar.*Hj;
                H_hat_copy(j,:) = (1/vj)*Hj;
                pAd = pinv(A(:,D));
                norm(H_hat_copy - pAd)
                [check_sum,sum_value, sum(sqrt(sum(pAd.^2, 2)))]
                keyboard
